...
@section('content')
    ...
    <div class="bg-[#3B0014]" x-data="servicesTabs()">
                                        ...
                                        <div class="h-[440px] overflow-hidden rounded-xl mb-6 relative">
                                            ...
                                            @if($service->image)
                                                <img src="{{ Storage::url($service->image) }}"
                                                    class="h-full object-cover object-center transition-all duration-500 ease-in-out w-full group-hover:scale-105"
                                                    alt="{{ $service->title }}">
                                            @else
                                                <img src="https://fastly.picsum.photos/id/3/5000/3333.jpg?hmac=GDjZ2uNWE3V59PkdDaOzTOuV3tPWWxJSf4fNcxu4S2g"
                                                    class="h-full object-cover object-center transition-all duration-500 ease-in-out w-full group-hover:scale-105"
                                                    alt="{{ $service->title }}">
                                            @endif
                                        </div>
        ...
        <!-- Modal Popup -->
        <div x-show="modalOpen" @click.self="closeModal()" x-transition:enter="transition ease-out duration-300"
            ...
            <div @click.stop x-transition:enter="transition ease-out duration-300"
                ...
                <div class="p-8">
                    <!-- Loading State -->
                    <div x-show="modalLoading" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                        x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
                        x-transition:leave-end="opacity-0" class="text-center py-6">
