...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <form action="{{ route('cms.services.store') }}" novalidate method="POST"  enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="mb-4" x-data="{ activeTab: 'key_benefits' }">
            ...
            <div x-show="activeTab === 'key_benefits'">
                <textarea name="key_benefits" id="key_benefits" rows="8" class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('key_benefits') }}</textarea>
            </div>
            <div x-show="activeTab === 'fees'">
                <textarea name="fees" id="fees" rows="8" class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('fees') }}</textarea>
            </div>
            <div x-show="activeTab === 'required_documents'">
                <textarea name="required_documents" id="required_documents" rows="8" class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('required_documents') }}</textarea>
            </div>
            <div x-show="activeTab === 'important_notes'">
                <textarea name="important_notes" id="important_notes" rows="8" class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('important_notes') }}</textarea>
            </div>
        </div>

        <div class="mb-4">
            <label for="consult_link" class="block text-sm font-medium text-gray-700 mb-2">Consult Link</label>
            <input type="url" name="consult_link" id="consult_link" value="{{ old('consult_link') }}" placeholder="https://example.com/consultation"
                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">
