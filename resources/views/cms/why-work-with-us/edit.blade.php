...
@section('title', 'Edit Card - Why Work With Us')
...
<div class="bg-white rounded-lg shadow-md p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Card - Why Work With Us</h1>

    <form action="{{ route('cms.why-work-with-us.update', $card) }}" novalidate method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4" required class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('description', $card->description) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            @if($card->image)
                <div class="mb-2">
                    <img src="{{ Storage::url($card->image) }}" alt="{{ $card->title }}" class="h-32 rounded">
                </div>
            @endif
