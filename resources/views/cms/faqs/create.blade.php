...
<div class="bg-white rounded-lg shadow-md p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Tambah FAQ</h1>

    <form action="{{ route('cms.faqs.store') }}" novalidate method="POST">
        @csrf
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="6" required class="wysiwyg w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
                ...
                <select name="page" id="page" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3B0014] focus:border-transparent">
                    ...
                    <option value="our-services" {{ old('page') == 'our-services' ? 'selected' : '' }}>Our Services</option>
                </select>
            </div>
            <div>
